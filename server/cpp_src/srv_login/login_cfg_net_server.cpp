...
void login_cfg_net_server::stop()
{
	if (cfg_session_)
		cfg_session_->close();
	work_ptr_.reset();
...
void login_cfg_net_server::tick()
{
	if (cfg_session_)
		cfg_session_->tick();
}
login_cfg_net_server::login_cfg_net_server()
{
...
void login_cfg_net_server::register_login2config_message()
{
#define REG_CONFIG_DISPATCHER(Msg, Function) dispatcher_manager_.register_dispatcher(new MsgDispatcher< Msg, login_cfg_session >(&login_cfg_session::Function));
...
void login_cfg_net_server::create_cfg_session(const std::string& ip, unsigned short port)
{
	auto session = std::make_shared<login_cfg_session>(io_service_);
	session->set_ip_port(ip, port);
	cfg_session_ = std::static_pointer_cast<base_net_session>(session);
}

void login_cfg_net_server::run()
{
...
void login_cfg_net_server::join()
{
	thread_.join();
}
