...
local hhdz_robot =require "game_script/all_game/hhdz_robot"
hhdz_table = virtual_table:new()
local hhdz_game = require "game_script/all_game/hhdz_game"
local MAX_COUNT = 20
local MAX_PLAYER_LIST = 100
local MAX_PLAYER_COUNT = 50
local MAX_TABLE_BET = 5000000
local hhdz_gamehelp = require "game_script/all_game/hhdz_gamehelp"
...
function post_msg_to_client_pb_sh(player,op_name,msg)
	if player.is_android then
		...
		if op_name == "SC_HongHeiPostResult" then
...
function hhdz_table:broadcast_msg_to_client_sh(op_name,msg)
...
function hhdz_table:init(room, table_id, chair_count)
	virtual_table.init(self, room, table_id, chair_count)
	self.table_status = HONGHEITABLESTATE.TABLE_WAIT_BETS
	self.status = HONGHEISTATE.TABLE_STSTE_INIT
	...
	self.robot_num = 0
	self.player_betinfo = {}  
	hhdz_game = hhdz_game:new()
	hhdz_game:init()

	hhdz_gamehelp = hhdz_gamehelp:new()
...
function hhdz_table:initialization(  )
	hhdz_game:initialization()
	for i=HONGHEIBET.BET_RED_WIN,HONGHEIBET.BET_LUCK_WIN do
		self.all_bets[i] = 0
		self.all_Player_bets[i] = 0
	end
	
	self.time0_ = os.time() + HONGHEITABLETIME.TABLE_BETS_TIME
	...
	self.cards[1] = {}
	self.cards[2] = {}
	self.card_type[1] = HONGHEITYPE.HONGHEI_GAOPAI
	self.card_type[2] = HONGHEITYPE.HONGHEI_GAOPAI
	self.table_init = 1


	local startmsg = {
	start_waittime = HONGHEITABLETIME.TABLE_BETS_TIME,
	playerbets = self.bet_base,
	allbets = self.all_bets,
	table_status = self.table_status,
}
	self:broadcast_msg_to_client_sh("SC_HongHeiStart",startmsg)
	--print_r(startmsg)
	log_info("hhdz_table:initialization")
	...
	hhdz_game:generatecards()

end

function hhdz_table:init_player( player ,isinit)
...
function hhdz_table:getplayerbets( player )
	local allbets = 0
	for i=HONGHEIBET.BET_RED_WIN,HONGHEIBET.BET_LUCK_WIN do
...
function hhdz_table:player_bet(player,money_,bettype)
	if money_ <= 0 or bettype < HONGHEIBET.BET_RED_WIN or bettype > HONGHEIBET.BET_LUCK_WIN  then
		post_msg_to_client_pb_sh(player,"SC_HongHeiPlayerBet",{playerbet_result=2,allbets=self.all_bets})
		return
	end
	...
	self:cost_money(player,{{money_type = pb.get_ev("ITEM_PRICE_TYPE", "ITEM_PRICE_TYPE_GOLD"), money =money_}}, pb.get_ev("LOG_MONEY_OPT_TYPE", "LOG_MONEY_OPT_TYPE_HONGHEI")) 
	...
	--post_msg_to_client_pb_sh(player,"SC_HongHeiPlayerBet",hongheiplayerbet)	

	self:broadcast_msg_to_client_sh("SC_HongHeiPlayerBet",hongheiplayerbet)
	--self:broadcast_msg_to_client_sh("SC_HongHeiAllBet",{allbets=self.all_bets})
	log_info("SC_HongHeiPlayerBet type "..bettype)
	--print_r(self.all_bets)
end

function hhdz_table:load_lua_cfg()
...
function hhdz_table:post_cards(player)
	--hhdz_game:generatecards()
	local postcard = {}
	postcard.result_waittime = self.time0_ - os.time()
	postcard.pb_card = {}
	if self.cheat_table.change_card > 0 then
		postcard.pb_card.red_cards = hhdz_game:getcard(HONGHEIBET.BET_BLACK_WIN)
		postcard.pb_card.black_cards = hhdz_game:getcard(HONGHEIBET.BET_RED_WIN)
	else
		postcard.pb_card.red_cards = hhdz_game:getcard(HONGHEIBET.BET_RED_WIN)
		postcard.pb_card.black_cards = hhdz_game:getcard(HONGHEIBET.BET_BLACK_WIN)
	end

	postcard.table_status = self.table_status

	if player ~= nil then
		post_msg_to_client_pb_sh(player,"SC_HongHeiPostCard",postcard)
				
	else
		self:broadcast_msg_to_client_sh("SC_HongHeiPostCard",postcard)
	end
	log_info("hhdz_table:post_cards to  ")
	print_r(postcard.pb_card)
end

function hhdz_table:post_result()
	local redcards 
	local blackcards 
	if self.cheat_table.change_card > 0 then
		redcards  = hhdz_game:getcard(HONGHEIBET.BET_BLACK_WIN)
		blackcards = hhdz_game:getcard(HONGHEIBET.BET_RED_WIN)
	else
		redcards  = hhdz_game:getcard(HONGHEIBET.BET_RED_WIN)
		blackcards = hhdz_game:getcard(HONGHEIBET.BET_BLACK_WIN)
	end
	self.game_log.redcards = redcards
	self.game_log.blackcards = blackcards

	local iswin,type1,type2 = hhdz_gamehelp.compare(redcards,blackcards)
	--local iswin,type1,type2 = hhdz_game:compare()
	self.card_type[1] = type1
	self.card_type[2] = type2
	local function is_jiabei(card)
		 local point = {}
		...
		point[3]    = math.floor(card[3] % 13) == 0 and 13 or math.floor(card[3] % 13)
		 if point[1] == point[2] then
			return  point[1] >=9 or point[1] ==1
		elseif point[1] == point[3] then
			return  point[1] >=9 or point[1] ==1 
		elseif point[2] == point[3] then
			return  point[2] >=9  or point[2] ==1  --1是对A >=9是9-K
		end 
	end 
	local record = {}
	local duizi_is_jiabei = true
	if iswin == 1 then
		record.win_color =1
		record.win_type =type1
		if record.win_type == HONGHEITYPE.HONGHEI_DUIZI then 
			duizi_is_jiabei = is_jiabei(redcards)
		end
		--log_info("redcards win")
	else
		record.win_color =2
		record.win_type =type2 
		if record.win_type == HONGHEITYPE.HONGHEI_DUIZI then 
			duizi_is_jiabei = is_jiabei(blackcards)
		end
		--log_info("blcakcards win")
	end
	
	 
	self.win_color = record.win_color
	...
	local result = {}
	result.next_waittime = HONGHEITABLETIME.TABLE_RESULT_TIME
	...
	for k,v in pairs(self.player_list_) do
		if v and  self.ready_list_[k] and v.bets ~= nil then
			...
			for i=HONGHEIBET.BET_RED_WIN,HONGHEIBET.BET_LUCK_WIN do
				if v.bets[i] > 0 then
					if i == iswin then
						v.win_money=math.floor(v.win_money + v.bets[i] * HONGHEIMULTIPLE[i])
					end

					if i == HONGHEIBET.BET_LUCK_WIN and record.win_type > HONGHEITYPE.HONGHEI_GAOPAI and duizi_is_jiabei then
						v.win_money=math.floor(v.win_money + v.bets[i] * HONGHEIMULTIPLE[record.win_type])
					end
				end
			end
			...
			if v.win_money > 0 then
				...
				v.taxes = result.tax
				self:add_money(v,{{money_type = pb.get_ev("ITEM_PRICE_TYPE", "ITEM_PRICE_TYPE_GOLD"), money = v.win_money-result.tax},tax=result.tax},  pb.get_ev("LOG_MONEY_OPT_TYPE", "LOG_MONEY_OPT_TYPE_HONGHEI"))
			...
			post_msg_to_client_pb_sh(v,"SC_HongHeiPostResult",result)


			if not(v.is_android) and curallbets > 0  then
				 log_info("hhdz_table:post_result to player ---------------"..v.guid)
...
-- }
    ...
    self:broadcast_msg_to_client_sh("SC_HongHeiRichList",richlist)
    log_info("hhdz_table:post_result")
...
function hhdz_table:statistics_rank( player )
	...
	self:broadcast_msg_to_client_sh("SC_HongHeiGameRecord",honghpostrecord)
	...
	if player ~= nil then
		post_msg_to_client_pb_sh(player,"SC_HongHeiPlayerList",onlineplayer)
		return
	end
	self:broadcast_msg_to_client_sh("SC_HongHeiPlayerList",onlineplayer)
end

function hhdz_table:sort_gamerecord( ... )
...
function hhdz_table:isinplayerlist(playerguid )
	for i,v in ipairs(self.player_list_) do
...
function hhdz_table:post_player_result(player)
	...
	post_msg_to_client_pb_sh(player,"SC_HongHeiPostResult",result)
	log_info("hhdz_table:post_player_result to player "..player.guid)		
...
function hhdz_table:cheatmode_bet()
	--hhdz_game:generatecards()
	...
	if self.time0_ - os.time() <= 1 then  --2秒倒计时
		...
		local redbets = self.all_Player_bets[HONGHEIBET.BET_RED_WIN]	--玩家投注
		local blackbets = self.all_Player_bets[HONGHEIBET.BET_BLACK_WIN]

		if redbets == blackbets then
			return
		end

		local all_redbets = self.all_bets[HONGHEIBET.BET_RED_WIN]	--所有的投注
		local all_blackbets = self.all_bets[HONGHEIBET.BET_BLACK_WIN]
		local luck_bets = self.all_bets[HONGHEIBET.BET_LUCK_WIN]*3 
		...
		if redbets > blackbets then
			...
			self.cheat_table.robot_betcolor = HONGHEIBET.BET_BLACK_WIN
		elseif redbets < blackbets then
			self.cheat_table.robot_cheatmoney = math.floor((blackbets - redbets) * 1.5 / self.robot_num/2)
			self.cheat_table.robot_betcolor = HONGHEIBET.BET_RED_WIN
		end

		local redcards  = hhdz_game:getcard(HONGHEIBET.BET_RED_WIN)
		local blackcards = hhdz_game:getcard(HONGHEIBET.BET_BLACK_WIN)
		local iswin,type1,type2 = hhdz_gamehelp.compare(redcards,blackcards)
		if iswin == 1 and self.cheat_table.robot_betcolor == HONGHEIBET.BET_RED_WIN then
			return
		elseif iswin == 2 and  self.cheat_table.robot_betcolor == HONGHEIBET.BET_BLACK_WIN then
			return
		end
...
function hhdz_table:tick()
	if self.table_status == HONGHEITABLESTATE.TABLE_WAIT_BETS then
		if self.status == HONGHEISTATE.TABLE_STSTE_INIT then
			self:initialization()
			self.status = HONGHEISTATE.TABLE_STSTE_CARDS 
		end
		self:cheatmode_bet()
		--log_info("initialization time "..os.time().."	time0_	"..self.time0_)
		if os.time()- self.time0_ >= 0 then
			self.table_status = HONGHEITABLESTATE.TABLE_POST_CARDS
			self.time0_ = os.time() + HONGHEITABLETIME.TABLE_CARDS_TIME
		end
	elseif self.table_status ==  HONGHEITABLESTATE.TABLE_POST_CARDS then
		if self.status == HONGHEISTATE.TABLE_STSTE_CARDS then
			self.status = HONGHEISTATE.TABLE_STSTE_RESULE 
			self:post_cards()
		end
		
		if os.time()- self.time0_ >= 0 then
			self.table_status = HONGHEITABLESTATE.TABLE_POST_RESULT
			self.time0_ = os.time() + HONGHEITABLETIME.TABLE_RESULT_TIME
		end

	elseif self.table_status ==  HONGHEITABLESTATE.TABLE_POST_RESULT then
		if self.status == HONGHEISTATE.TABLE_STSTE_RESULE then
			self.status = HONGHEISTATE.TABLE_STSTE_REST
			self:post_result()
		end

		if os.time()- self.time0_ >= 0 then
			self.table_status = HONGHEITABLESTATE.TABLE_REST
			self.time0_ = os.time() + HONGHEITABLETIME.TABLE_REST_TIME
		end


	elseif self.table_status ==  HONGHEITABLESTATE.TABLE_REST then
		if self.status == HONGHEISTATE.TABLE_STSTE_REST then
			self.status = HONGHEISTATE.TABLE_STSTE_INIT
			self:table_rest()
		end
...
function hhdz_table:table_rest(player )
	...
	self:broadcast_msg_to_client_sh("SC_HongHeiRest",{rest_waittime=(self.time0_ - os.time())})
...
function hhdz_table:isPlay(player)
	if player then 
		if self.table_status ~= HONGHEITABLESTATE.TABLE_REST then 
...
function hhdz_table:reconnection_client_msg(player)
...
function hhdz_table:notify_offline(player)
...
function hhdz_table:reconnect(player)
    player.is_offline = false
end


function hhdz_table:check_cancel_ready(player, is_offline)
...
-- 检查是否可准备   
function hhdz_table:check_ready(player)
	self:init_player(player)
	self:send_reconnect_player(player,false)
	return not self:isPlay()
end

function hhdz_table:send_reconnect_player(player,isreconnet )
	local reconnect = {}
	...
	if self.table_status >  HONGHEITABLESTATE.TABLE_WAIT_BETS then
		reconnect.pb_card.red_cards = hhdz_game:getcard(HONGHEIBET.BET_RED_WIN)
		reconnect.pb_card.black_cards =hhdz_game:getcard(HONGHEIBET.BET_BLACK_WIN)
	end
	...
	post_msg_to_client_pb_sh(player,"SC_HongHeiReConnect",reconnect)

	if self.table_status >  HONGHEITABLESTATE.TABLE_POST_CARDS then
...
function hhdz_table:send_data_to_enter_player(player,isreconnet )
	if self.table_status == HONGHEITABLESTATE.TABLE_WAIT_BETS then
		local startmsg = {
		...
		post_msg_to_client_pb_sh(player,"SC_HongHeiStart",startmsg)
		log_info("hhdz_table:SC_HongHeiStart to player "..player.guid)		
	elseif self.table_status ==  HONGHEITABLESTATE.TABLE_POST_CARDS then
...
function hhdz_table:remove_playerlist( player )
	--for i,j in ipairs(self.playerrecord) do
...
function hhdz_table:cost_money( player,price, opttype )
...
function hhdz_table:add_money( player,price, opttype )
...
-- function hhdz_table:player_stand_up(player, is_offline)
