...
local longhu_robot = {}
setmetatable(longhu_robot, {__index = virtual_player})
local robot_num= 25
...
function longhu_robot:new()  
	local o = {}
    setmetatable(o, {__index = longhu_robot})
    o.timer = {}
    return o 
end

function longhu_robot:forced_exit()
...
function longhu_robot:game_msg( msg_name, msg )
	if msg_name == "SC_LongHuStart" then
...
function longhu_robot:robot_bet( )
    math.randomseed(get_second_time()+self.guid+self:get_money())
...
function longhu_robot:robot_cheatbet(bettype,betmoney )
    while 1 do
        for i=5,1,-1 do
