...
require "game_script/all_game/brnn_table"
local OX_STATUS_FREE = 1
local OX_GET_TOP_DIS = 10
local TOP_MAX_NUM =300
brnn_room_mgr = virtual_room_mgr:new()
function brnn_room_mgr:get_top_info(player)
...
function brnn_room_mgr:auto_sit_down(player)
...
function brnn_room_mgr:sit_down(player, table_id_, chair_id_)
...
function brnn_room_mgr:stand_up(player)
...
function brnn_room_mgr:init(tb, chair_count, ready_mode, room_lua_cfg)
	virtual_room_mgr.init(self, tb, chair_count,ready_mode,room_lua_cfg)
	self.last_get_top_time =0
	self.count_all_player =0
	self.top_info = {}
end
function brnn_room_mgr:create_table()
	return brnn_table:new()
end
function brnn_room_mgr:on_sit_down(player)
	local tb = self:get_user_table(player)
	if tb then
		local chat = {
...
function brnn_room_mgr:player_offline(player)
