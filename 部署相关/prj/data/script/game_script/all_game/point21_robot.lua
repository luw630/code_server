...
setmetatable(point21_robot, {__index = virtual_player})
...
function ly_robot_mgr.check_table(tb)
    ...
    if def_second_game_type ~= 99 then
        ...
        if  p_count < 1 and tb:has_real_player() then
            if math.random(ly_add_robot_random_time) == 1 then
                ly_robot_mgr.add_a_robot(tb,nil)
            end
...
function ly_robot_mgr.add_a_robot(tb,def_chair_id)
        local player = point21_robot.new()
        if not def_chair_id then
            ...
            tb.player_list_[def_chair_id] = player
            player.chair_id = def_chair_id 
        end
        ...
        if Rich_tag == 1 or def_chair_id then 
            Rich_tag =5000
        else 
            Rich_tag =800
        end 
...
function point21_robot:forced_exit()
...
function point21_robot.new()
	local o = {}
    setmetatable(o, {__index = point21_robot})
    o.timer = {}
    return o 
end

function point21_robot:game_msg(msg_name, msg)
    if "SC_Point21_Desk_Enter" == msg_name then --开始
  
    elseif "SC_Point21_Game_Finish" == msg_name then
	elseif "GAME_CLOSE" == msg_name then
		local rt =  math.random(1,100)
        if rt < 10 and self.chair_id ~= self.tb.zhuang_chair_id then
			self:forced_exit()--机器人退出
		else
            if self.chair_id == self.tb.zhuang_chair_id then
                self.pb_base_info.money = math.random(100,5000)*self.tb.cell_score_
            end
            local act = {}
